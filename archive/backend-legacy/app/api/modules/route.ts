...
import { ensureRegistryModules, getUserWorkspaceId, updateRegistryModuleEnabled } from "@/supabase/queries/modulesRegistry";
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const data = await updateRegistryModuleEnabled(client, workspaceId, input.id, input.enabled);
    return NextResponse.json({ data });
  } catch (error) {
    ctx.error("failed", { error: error instanceof Error ? error.message : String(error) });
    return toErrorResponse(error, ctx.requestId);
