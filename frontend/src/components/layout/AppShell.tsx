import { Bell, CircleUser, CreditCard, FileText, LogOut, Server, Settings } from "lucide-react";
...
function SidebarItem({ to, title, icon, end = false }: ItemProps) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `flex h-11 w-full items-center gap-3 rounded-2xl px-3 text-sm outline-none transition-all duration-300 focus-visible:ring-2 focus-visible:ring-white/40 ${
          isActive
      ...
      <div className="grid min-h-screen grid-cols-[64px_1fr]">
        <aside className="group/sidebar sticky top-0 z-30 flex h-screen w-16 flex-col overflow-hidden rounded-r-3xl bg-brand-500 px-2 py-4 transition-all duration-300 hover:w-56 dark:bg-[#1A1A1A]">
          <NavLink
            ...
            title="Notifications"
            className={({ isActive }) =>
              `${iconButton} mb-4 ${isActive ? "bg-white text-brand-500 shadow-panel" : ""}`
            }
          >
            <Bell size={18} />
            <span className="pointer-events-none whitespace-nowrap opacity-0 transition-opacity duration-200 group-hover/sidebar:opacity-100">
          ...
          <nav className="flex flex-1 flex-col gap-3">
            ...
            <SidebarItem to="/settings/billing" title="Billing" icon={<CreditCard size={18} />} end />
