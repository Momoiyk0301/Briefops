...
async function sendWaitlistConfirmationEmail(email: string) {
  if (!RESEND_API_KEY || !MAIL_FROM) {
    throw new Error("Resend non configure: RESEND_API_KEY et MAIL_FROM sont requis");
  }
  ...
  if (!resendRes.ok) {
    ...
    throw new Error(`Resend a refuse l'envoi (${resendRes.status}): ${errorBody}`);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[waitlist] Email confirmation failed", {
      email,
      error: error instanceof Error ? error.message : String(error)
    });
    return NextResponse.json({ error: "Inscription enregistree, mais email de confirmation non envoye" }, { status: 502 });
  }
