...
vi.mock("@/env", () => ({
  env: { APP_URL: "https://briefing.events-ops.be" }
}));
...
describe("frontend /api/briefings/:id/share", () => {
  ...
  it("lists and creates staff or audience public links", async () => {
    ...
    expect(getBody.data[0].url).toBe("https://briefing.events-ops.be/briefing/share/t1");
    expect(getBody.data[1].url).toBe("https://briefing.events-ops.be/briefing/share/t2");
  });

  it("creates an audience link with a normalized team key", async () => {
    const client = {
      from: () => {
        const chain = {
          select: () => chain,
    ...
    expect(postRes.status).toBe(201);
    expect(createPublicLink).toHaveBeenCalledTimes(1);
    expect(createPublicLink.mock.calls[0][4]).toBe("audience");
    expect(createPublicLink.mock.calls[0][5]).toBe("sound");
    const body = await postRes.json();
    expect(body.data.url).toBe("https://briefing.events-ops.be/briefing/share/team-token");
  });
