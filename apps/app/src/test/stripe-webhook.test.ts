...
const profileUpsertMaybeSingle = vi.fn();
const profileUpdate = vi.fn();
const profileUpdateMaybeSingle = vi.fn();
...
describe("stripe webhook integration", () => {
  beforeEach(() => {
    ...
    profileUpsert.mockImplementation(() => ({
      select: () => ({
        maybeSingle: profileUpsertMaybeSingle
      })
    }));
    profileUpsertMaybeSingle.mockResolvedValue({
    ...
    profileUpdateMaybeSingle.mockResolvedValue({
      data: { id: "user-1", email: "client@example.com" },
      error: null
    });
    ...
    mockAdminFrom.mockImplementation((table: string) => {
      ...
      if (table === "workspaces") {
        return {
          select: () => ({
            eq: (field: string) => ({
              maybeSingle: field === "owner_id" ? ownerWorkspaceMaybeSingle : workspaceMaybeSingle
            })
          }),
          insert: () => ({
            select: () => ({
  ...
  it("uses the central mail service and stores Stripe state on the profile", async () => {
