...
import { buildAppUrl, getMarketingSiteUrl } from "@shared/sites";
...
export async function sendTransactionalEmail(input: SendMailInput & { kind?: AppEmailKind; tags?: Record<string, string> }) {
  ...
  if (!config) {
    if (isDev) {
      logEvent("info", "[mail] skipped: RESEND_API_KEY or MAIL_FROM missing", { kind: input.kind ?? "unknown" });
    }
    return;
  }
...
function renderMailShell(title: string, content: string, ctaLabel?: string, ctaHref?: string) {
  ...
  return `
    <div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827;max-width:640px;margin:0 auto;padding:24px;">
      ...
      ${action}
      <p style="margin-top:32px;font-size:12px;color:#6b7280;">BriefOPS · ${escapeHtml(new URL(getMarketingSiteUrl()).host)}</p>
    </div>
  `;
}

export async function sendCheckoutConfirmationEmails(email: string, plan: BillingPlan, session: Stripe.Checkout.Session) {
  const dashboardUrl = buildAppUrl("/briefings");
