...
const SELECT_PUBLIC_LINK_FIELDS =
  "id, briefing_id, resource_type, link_type, audience_tag, token, created_by, expires_at, revoked_at, created_at, views_count";
...
export async function incrementViewsCount(client: SupabaseClient, linkId: string) {
...
export async function resolvePublicLinkByToken(client: SupabaseClient, token: string) {
  ...
  const audienceTag = link.link_type === "audience" ? (link.audience_tag ?? null) : null;
  const payload = await getPublicBriefingPayload(client, link.briefing_id);

  const filteredModules = audienceTag
    ? (payload.modules ?? []).filter((mod) => {
        const dataJson = mod.data_json as Record<string, unknown> | null;
        const audience = dataJson?.audience as { mode?: string; teams?: string[] } | null;
        if (!audience) return true;
        if (audience.mode === "all") return true;
        return Array.isArray(audience.teams) && audience.teams.includes(audienceTag);
      })
    : payload.modules;
  ...
  return {
    ...
    link,
    audienceTag,
    resolvedView: link.link_type === "staff" ? ("staff" as const) : ("audience" as const)
  };
}
